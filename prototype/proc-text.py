...
ds_train, ds_validate = train_test_split(
    df_train, test_size=0.1, random_state=42, stratify=df_train["pattern"].values
)
...
text_clf = Pipeline(
    [
        ...
        (
            "clf",
            SGDClassifier(
                loss="modified_huber", penalty="elasticnet", alpha=1e-3, random_state=42
            ),
        ),
    ]
)
...
grid_search = GridSearchCV(
    text_clf, param_grid=parameters, n_jobs=-1, cv=skf, verbose=1
)
